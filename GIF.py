...
class GIF(QThread):
    ...
    def screencut(self,offset_x = 0, offset_y = 0, width = 1920, height = 1080):
        filename = "/home/arch/wallpaper/pictures/screencut/" + str(int(time.time() * 1000))+".png"
        res = subprocess.call("ffmpeg -f x11grab -video_size "+ str(width) +"x"+ str(height) +" -i :0.0+"+  str(offset_x) +","+ str(offset_y) +" -frames:v 1 " + filename, shell=True)
        if res:
            temp = ["failed!","截图失败"]
        else:
    ...
    def begingif(self,offset_x = 0, offset_y = 0, width = 1920, height = 1080, fps = 30, draw_mouse = 0):
        filename = "/home/arch/wallpaper/pictures/screencut/" + str(int(time.time() * 1000))+".mp4"
        self.process = subprocess.Popen("ffmpeg -video_size "+ str(width) +"x"+ str(height) +" -framerate 15 -f x11grab -i :0.0+"+ str(offset_x) +","+ str(offset_y)+" "+filename,shell=True, stdin=subprocess.PIPE)
